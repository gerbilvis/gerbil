...
*/
char* vise_basename( const char* pathname )
{
  char* base;
  ...
  if( ! last_slash )
    {
      base = static_cast<char *>(calloc( strlen( pathname ) + 1, sizeof( char ) ));
      strcpy( base, pathname );
    }
  else
    {
      base = static_cast<char *>(calloc( strlen( last_slash++ ), sizeof( char ) ));
