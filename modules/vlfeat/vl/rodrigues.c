 ...
 ** @param R_pt   3x3  matrix - array of 9   double (in).
 **
 ** FIXME adapted by Christian Riess w/o checking what it does. Search for FIXME below
 **
...
#define W(i,j)   W_pt[(i)+3*(j)]
  ...
  if(fabs(sth) < small && cth < 0) {
    ...
    */
    ...
    double W_pt [9], x, y, z ;
	/*
	// FIXME ORIGINAL CODE - LOOKS VERY MUCH LIKE A COPY-PASTE ISSUE. NEW VERSION IS BELOW. NOTE THAT THE CODE IS NOT CHECKED.
...
*/
	// FIXME "corrected" version, w/o digging into the computation
    W_pt[0] = 0.5*( R(0,0) + R(0,0) ) - 1.0 ;    
