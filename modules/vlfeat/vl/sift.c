 ...
 vl_sift_pix const *src, int width, int height)
{
  ...
  for(y = 0 ; y < height ; ++y) {
    ...
    for(x = 0 ; x < width - 1 ; ++x) {
      b = *src++ ;
      *dst = a ;             dst += height ;
      *dst = (vl_sift_pix)(0.5 * (a + b)) ; dst += height ;
      a = b ;
    }
    *dst = b ; dst += height ;
    *dst = b ; dst += height ;
...
#define SAVE_BACK                                                       \
    *grad++ = vl_fast_sqrt_f ((float)(gx*gx + gy*gy)) ;                          \
    *grad++ = vl_mod_2pi_f   (vl_fast_atan2_f (gy, gx) + (float)(2*VL_PI)) ;     \
    ++src ;                                                             \
    ...
    while (src < end) {
      gx = (float)(0.5 * (src[+xo] - src[-xo])) ;
      gy = (float)(       src[+yo] - src[0]) ;
      SAVE_BACK ;
    }
    ...
    for (y = 1 ; y < h -1 ; ++y) {
      ...
      gx =        src[+xo] - src[0] ;
      gy = (vl_sift_pix)(0.5 * (src[+yo] - src[-yo])) ;
      SAVE_BACK ;

      /* middle pixels of the middle rows */
      end = (src - 1) + w - 1 ;
      while (src < end) {
        gx = (vl_sift_pix)(0.5 * (src[+xo] - src[-xo])) ;
        gy = (vl_sift_pix)(0.5 * (src[+yo] - src[-yo])) ;
        SAVE_BACK ;
      }
...
#define atd(dbinx,dbiny,dbint) *(dpt + (dbint)*binto + (dbiny)*binyo + (dbinx)*binxo)
  ...
  for(dyi =  VL_MAX(- W,   - yi   ) ;
    ...
    for(dxi =  VL_MAX(- W,   - xi   ) ;
      ...
      vl_sift_pix theta = vl_mod_2pi_f ((float)(angle - angle0)) ;
      ...
      int         binx = vl_floor_f ((float)(nx - 0.5)) ;
      int         biny = vl_floor_f ((float)(ny - 0.5)) ;
  ...
  if(1) {
    ...
    if(f-> norm_thresh && norm < f-> norm_thresh * numSamples) {
      ...
      /* truncate at 0.2. */
      for(bin = 0; bin < NBO*NBP*NBP ; ++ bin) {
        if (descr [bin] > 0.2) descr [bin] = (vl_sift_pix)0.2;
      }
