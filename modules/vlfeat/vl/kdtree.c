 ...
 unsigned int depth)
{
  ...
  if (dataEnd - dataBegin <= 1) {
    if (tree->depth < (int)depth) tree->depth = depth ;
    node->lowerChild = - (int)dataBegin - 1;
    node->upperChild = - (int)dataEnd - 1 ;
    return ;
  }
  ...
  for (d = 0 ; d < forest->numDimensions ; ++ d) {
    ...
    float variance = 0 ;
    for (i = (int)dataBegin ; i < (int)dataEnd ; ++ i) {
  ...
  switch (forest->thresholdingMethod) {
    case VL_KDTREE_MEAN :
      ...
      for (splitIndex = dataBegin ;
           splitIndex < (int)dataEnd && tree->dataIndex[splitIndex].value <= node->splitThreshold ;
           ++ splitIndex) ;
      splitIndex -= 1 ;
       ...
       * points. */
      if ((((int)dataBegin) <= splitIndex) && (splitIndex + 1 < (int)dataEnd)) break ;
...
vl_kdforest_delete (VlKDForest * self)
{
  ...
  if (self->trees) {
    for (ti = 0 ; ti < (int)self->numTrees ; ++ ti) {
      if (self->trees[ti]) {
                   ...
                   float const * query)
{
  ...
  for (i = (int)numAddedNeighbors ;  i < (int)numNeighbors ; ++ i) {
    neighbors[i].index = -1 ;
