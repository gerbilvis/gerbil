...
void getCountsWrap(cl::Buffer& counts, ocl_charMatrix& ir, ocl_intMatrix& sums)
{
    ...
    for(unint numDone = 0; numDone < ir.pr; numDone += todo)
    {
        ...
        queue.enqueueNDRangeKernel(getCountsKernel,
                  ...
                  ocl_intMatrix& sums, unint offSet)
{
    ...
    for(unint numDone = 0; numDone < ir.r; numDone += todo)
    {
...
//  dAux = (intMatrix*)calloc( depth+1, sizeof(*dAux) );
    ...
    while(numDone < in.r)
    {
        ...
        DBG_DEVICE_MATRIX_WRITE(sum, "dSums_internal.txt");
        DBG_DEVICE_MATRIX_WRITE(dAux[0], "dAux[0]_internal.txt");
        ...
        for(i=0; i < depth; i++)
        {
            ...
            DBG_DEVICE_MATRIX_WRITE(dAux[0], "dAux[0]_internal_2.txt");
            DBG_DEVICE_MATRIX_WRITE(dAux[1], "dAux[1]_internal_2.txt");
...
//          cudaThreadSynchronize();
       ...
       // cudaThreadSynchronize();
        ...
        DBG_DEVICE_MATRIX_WRITE(dAux[0], "dAux[0]_internal_final.txt");
        DBG_DEVICE_MATRIX_WRITE(sum, "dSums_internal_1.txt");
