...
void Viewport::updateBuffers(RenderMode spectrum, RenderMode highlight)
{
	...
	for (int i = 0; i < 2; ++i) {
		...
		StopwatchGPU watch(QString("%1\tupdateBuffer %2 %3")
						   .arg(representation::str(type))
						   .arg(i == 1 ? "FORE" : "BACK")
						   .arg(mode[i] == RM_FULL ? "FULL" : "PART"));
...
void Viewport::continueDrawing(int buffer)
{
	...
	StopwatchGPU watch(QString("%1\tcontinueBuffer %2")
					   .arg(representation::str(type))
					   .arg(buffer == 1 ? "FORE" : "BACK"));
	QPainter painter(b.fbo);
